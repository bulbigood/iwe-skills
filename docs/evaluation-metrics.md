...
- `weak` requires `90%` success for task correctness and evidence quality, `100%` for safety, and `80%` for scenario/skill compliance and tool/resource efficiency. Every value is declared explicitly in `config.toml`; the runner has no implicit fallback map.
