# Raw IWE 0.18.0 CLI help inventory

Generated from the exact configured binary `/home/linuxbrew/.linuxbrew/Cellar/iwe/0.18.0/bin/iwe`. Maintainer research only; this is not part of the installed skill payload.
...
Options:
  ...
  -v, --verbose <VERBOSE>              [default: 0]
      ...
      --explain                        Print the binding trace instead of validating
      --filter <FILTER>                Inline YAML filter expression
  -k, --key <KEY>                      Match by document key; repeatable
      --includes <INCLUDES>            Inclusion anchor KEY or KEY:DEPTH; repeatable
      --included-by <INCLUDED_BY>      Reverse inclusion anchor KEY or KEY:DEPTH; repeatable
      --references <REFERENCES>        Reference anchor KEY or KEY:DIST; repeatable
      --referenced-by <REFERENCED_BY>  Reverse reference anchor KEY or KEY:DIST; repeatable
      --roots                          Only match root documents
      --max-depth <MAX_DEPTH>          Default inclusion depth; 0 is unbounded
      --max-distance <MAX_DISTANCE>    Default reference distance; 0 is unbounded
  -h, --help                           Print help
```

## iwe stats similarity --help
...
Options:
  -t, --threshold <THRESHOLD>  Match level a pair must clear in both directions [default: 0.85]
  -v, --verbose <VERBOSE>      [default: 0]
  -h, --help                   Print help
```
